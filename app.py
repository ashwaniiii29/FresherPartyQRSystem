"""
Fresher Party QR Registration System
"""

import os
import re
import sqlite3
import uuid
import shutil
from PIL import Image, ImageDraw, ImageFont
from datetime import datetime

import qrcode
from flask import (
    Flask, render_template, request, redirect,
    url_for, session, send_from_directory, flash
)

app = Flask(__name__)
app.secret_key = "fresher-party-2026-secret-key-change-me"

BASE_DIR = os.path.dirname(os.path.abspath(__file__))
DB_PATH = os.path.join(BASE_DIR, "participants.db")
QR_FOLDER = os.path.join(BASE_DIR, "qr_codes")

os.makedirs(QR_FOLDER, exist_ok=True)

EVENT_NAME = "Fresher Party 2026"

ADMIN_USERNAME = "admin"
ADMIN_PASSWORD = "admin123"


def get_db_connection():
    conn = sqlite3.connect(DB_PATH)
    conn.row_factory = sqlite3.Row
    return conn


def init_db():
    conn = get_db_connection()

    conn.execute("""
        CREATE TABLE IF NOT EXISTS participants (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            registration_id TEXT UNIQUE NOT NULL,
            name TEXT NOT NULL,
            email TEXT UNIQUE NOT NULL,
            mobile TEXT UNIQUE NOT NULL,
            course TEXT NOT NULL,
            year TEXT NOT NULL,
            gender TEXT NOT NULL,
            student_id TEXT UNIQUE NOT NULL,
            qr_filename TEXT NOT NULL,
            registration_date TEXT NOT NULL,
            checked_in INTEGER DEFAULT 0,
            checkin_time TEXT
        )
    """)

    conn.commit()
    conn.close()

def backup_database():
    if not os.path.exists(DB_PATH):
        return

    backup_folder = os.path.join(BASE_DIR, "backups")
    os.makedirs(backup_folder, exist_ok=True)

    timestamp = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")

    backup_path = os.path.join(
        backup_folder,
        f"participants_backup_{timestamp}.db"
    )

    shutil.copy2(DB_PATH, backup_path)


EMAIL_REGEX = re.compile(r"^[^\s@]+@[^\s@]+\.[^\s@]+$")


def validate_registration_form(form):
    errors = []

    name = form.get("name", "").strip()
    email = form.get("email", "").strip()
    mobile = form.get("mobile", "").strip()
    course = form.get("course", "").strip()
    year = form.get("year", "").strip()
    gender = form.get("gender", "").strip()
    student_id = form.get("student_id", "").strip()

    if not name:
        errors.append("Name is required.")
    elif len(name) < 2:
        errors.append("Name must be at least 2 characters long.")

    if not email:
        errors.append("Email is required.")
    elif not EMAIL_REGEX.match(email):
        errors.append("Please enter a valid email address.")

    if not mobile:
        errors.append("Mobile number is required.")
    elif not (mobile.isdigit() and len(mobile) == 10):
        errors.append("Mobile number must contain exactly 10 digits.")

    if not course:
        errors.append("Course is required.")

    if not year:
        errors.append("Year/Semester is required.")

    if not gender:
        errors.append("Gender is required.")

    if not student_id:
        errors.append("College/Student ID is required.")

    return errors


def is_duplicate(email, mobile, student_id):
    conn = get_db_connection()

    row = conn.execute(
        """
        SELECT * FROM participants
        WHERE email = ? OR mobile = ? OR student_id = ?
        """,
        (email, mobile, student_id),
    ).fetchone()

    conn.close()

    return row is not None


def generate_qr_code(registration_id, name, email, mobile, course, year, student_id):

    qr_data = registration_id

    qr_img = qrcode.make(qr_data)

    filename = f"{registration_id}.png"

    filepath = os.path.join(QR_FOLDER, filename)

    qr_img.save(filepath)

    return filename


def generate_registration_id():
    conn = get_db_connection()

    row = conn.execute(
        """
        SELECT MAX(CAST(SUBSTR(registration_id, 5) AS INTEGER)) AS last_number
        FROM participants
        WHERE registration_id LIKE 'FP26-%'
        """
    ).fetchone()

    conn.close()

    last_number = row["last_number"] or 0
    next_number = last_number + 1

    return f"FP26-{next_number:04d}"


@app.route("/", methods=["GET", "POST"])
def index():

    if request.method == "POST":

        errors = validate_registration_form(request.form)

        name = request.form.get("name", "").strip()
        email = request.form.get("email", "").strip().lower()
        mobile = request.form.get("mobile", "").strip()
        course = request.form.get("course", "").strip()
        year = request.form.get("year", "").strip()
        gender = request.form.get("gender", "").strip()
        student_id = request.form.get("student_id", "").strip()

        if not errors and is_duplicate(
            email,
            mobile,
            student_id
        ):
            errors.append(
                "A participant with this email, mobile number, "
                "or student ID has already registered."
            )

        if errors:
            return render_template(
                "index.html",
                event_name=EVENT_NAME,
                errors=errors,
                form_data=request.form,
            )

        registration_id = generate_registration_id()

        qr_filename = generate_qr_code(
            registration_id,
            name,
            email,
            mobile,
            course,
            year,
            student_id
        )

        registration_date = datetime.now().strftime(
            "%Y-%m-%d %H:%M:%S"
        )

        conn = get_db_connection()

        conn.execute(
            """
            INSERT INTO participants
            (
                registration_id,
                name,
                email,
                mobile,
                course,
                year,
                gender,
                student_id,
                qr_filename,
                registration_date
            )
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """,
            (
                registration_id,
                name,
                email,
                mobile,
                course,
                year,
                gender,
                student_id,
                qr_filename,
                registration_date
            ),
        )

        conn.commit()
        conn.close()

        backup_database()

        return redirect(
            url_for(
                "success",
                registration_id=registration_id
            )
        )

    return render_template(
        "index.html",
        event_name=EVENT_NAME,
        errors=[],
        form_data={}
    )


@app.route("/success/<registration_id>")
def success(registration_id):

    conn = get_db_connection()

    participant = conn.execute(
        """
        SELECT * FROM participants
        WHERE registration_id = ?
        """,
        (registration_id,)
    ).fetchone()

    conn.close()

    if participant is None:
        flash("Registration not found.")
        return redirect(url_for("index"))

    return render_template(
        "success.html",
        participant=participant,
        event_name=EVENT_NAME
    )


@app.route("/qr_codes/<filename>")
def qr_image(filename):
    return send_from_directory(
        QR_FOLDER,
        filename
    )

@app.route("/download_pass/<registration_id>")
def download_pass(registration_id):

    conn = get_db_connection()

    participant = conn.execute(
        """
        SELECT * FROM participants
        WHERE registration_id = ?
        """,
        (registration_id,)
    ).fetchone()

    conn.close()

    if participant is None:
        flash("Registration not found.")
        return redirect(url_for("index"))

    qr_path = os.path.join(
        QR_FOLDER,
        participant["qr_filename"]
    )

    qr_img = Image.open(qr_path).convert("RGB")

    width = qr_img.width
    height = qr_img.height

    pass_height = height + 180

    pass_img = Image.new(
        "RGB",
        (width, pass_height),
        "white"
    )

    pass_img.paste(
        qr_img,
        (0, 20)
    )

    draw = ImageDraw.Draw(pass_img)

    try:
        font = ImageFont.truetype(
            "arial.ttf",
            32
        )
        small_font = ImageFont.truetype(
            "arial.ttf",
            24
        )
    except:
        font = ImageFont.load_default()
        small_font = ImageFont.load_default()

    name = participant["name"]
    registration_id = participant["registration_id"]

    name_box = draw.textbbox(
        (0, 0),
        name,
        font=font
    )

    name_width = name_box[2] - name_box[0]

    draw.text(
        (
            (width - name_width) // 2,
            height + 35
        ),
        name,
        fill="black",
        font=font
    )

    reg_text = f"Registration ID: {registration_id}"

    reg_box = draw.textbbox(
        (0, 0),
        reg_text,
        font=small_font
    )

    reg_width = reg_box[2] - reg_box[0]

    draw.text(
        (
            (width - reg_width) // 2,
            height + 90
        ),
        reg_text,
        fill="black",
        font=small_font
    )

    filename = f"{registration_id}_pass.png"

    filepath = os.path.join(
        QR_FOLDER,
        filename
    )

    pass_img.save(filepath)

    return send_from_directory(
        QR_FOLDER,
        filename,
        as_attachment=True
    )

def admin_logged_in():
    return session.get("is_admin", False)


@app.route("/admin/login", methods=["GET", "POST"])
def admin_login():

    error = None

    if request.method == "POST":

        username = request.form.get(
            "username",
            ""
        ).strip()

        password = request.form.get(
            "password",
            ""
        ).strip()

        if (
            username == ADMIN_USERNAME
            and password == ADMIN_PASSWORD
        ):
            session["is_admin"] = True

            return redirect(
                url_for("admin_dashboard")
            )

        else:
            error = "Invalid username or password."

    return render_template(
        "admin_login.html",
        error=error,
        event_name=EVENT_NAME
    )


@app.route("/admin/logout")
def admin_logout():

    session.pop("is_admin", None)

    return redirect(
        url_for("admin_login")
    )


@app.route("/admin")
def admin_dashboard():

    if not admin_logged_in():
        return redirect(
            url_for("admin_login")
        )

    search = request.args.get(
        "search",
        ""
    ).strip()

    conn = get_db_connection()

    if search:

        like = f"%{search}%"

        participants = conn.execute(
            """
            SELECT * FROM participants
            WHERE name LIKE ?
               OR registration_id LIKE ?
               OR email LIKE ?
               OR mobile LIKE ?
               OR student_id LIKE ?
            ORDER BY id DESC
            """,
            (
                like,
                like,
                like,
                like,
                like
            ),
        ).fetchall()

    else:

        participants = conn.execute(
            """
            SELECT * FROM participants
            ORDER BY id DESC
            """
        ).fetchall()

    total = conn.execute(
        """
        SELECT COUNT(*) AS c
        FROM participants
        """
    ).fetchone()["c"]

    checked_in_count = conn.execute(
        """
        SELECT COUNT(*) AS c
        FROM participants
        WHERE checked_in = 1
        """
    ).fetchone()["c"]

    conn.close()

    return render_template(
        "admin.html",
        participants=participants,
        total=total,
        checked_in_count=checked_in_count,
        search=search,
        event_name=EVENT_NAME
    )


@app.route("/admin/checkin", methods=["GET", "POST"])
def checkin():

    if not admin_logged_in():
        return redirect(
            url_for("admin_login")
        )

    participant = None
    message = None
    message_type = None

    if request.method == "POST":

        registration_id = request.form.get(
            "registration_id",
            ""
        ).strip().upper()

        conn = get_db_connection()

        participant = conn.execute(
            """
            SELECT * FROM participants
            WHERE registration_id = ?
            """,
            (registration_id,)
        ).fetchone()

        if participant is None:

            message = (
                f"No participant found with "
                f"Registration ID '{registration_id}'."
            )

            message_type = "error"

        else:

            action = request.form.get(
                "action"
            )

            if action == "mark_checked_in":

                if participant["checked_in"]:

                    message = (
                        f"{participant['name']} "
                        f"is already checked in."
                    )

                    message_type = "info"

                else:

                    checkin_time = datetime.now().strftime(
                        "%Y-%m-%d %H:%M:%S"
                    )

                    conn.execute(
                        """
                        UPDATE participants
                        SET checked_in = 1,
                            checkin_time = ?
                        WHERE registration_id = ?
                        """,
                        (
                            checkin_time,
                            registration_id
                        )
                    )

                    conn.commit()

                    participant = conn.execute(
                        """
                        SELECT * FROM participants
                        WHERE registration_id = ?
                        """,
                        (registration_id,)
                    ).fetchone()

                    message = (
                        f"{participant['name']} "
                        f"has been checked in successfully!"
                    )

                    message_type = "success"

        conn.close()

    return render_template(
        "checkin.html",
        participant=participant,
        message=message,
        message_type=message_type,
        event_name=EVENT_NAME
    )


if __name__ == "__main__":
    init_db()

    app.run(
        debug=True
    )